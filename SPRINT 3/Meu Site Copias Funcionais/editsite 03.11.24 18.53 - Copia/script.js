document.addEventListener("DOMContentLoaded", function () {
    ...
    // Função de calendário com possibilidade de adicionar notas
    function generateCalendar(month, year) {
        ...
        // Cria os dias do mês e adiciona o evento de clique
        for (let day = 1; day <= daysInMonth; day++) {
            const dayElement = document.createElement("div");
            dayElement.textContent = day;
            dayElement.classList.add("calendar-day");

            // Evento de clique para abrir uma nota para o dia selecionado
    ...
    // Função para adicionar uma nota para uma data específica
    function addNoteForDate(date) {
        ...
        if (note) {
            ...
            // Exibe a data e a nota
            noteItem.innerHTML = `<strong>${date}:</strong> ${note} `;

            // Cria um botão de exclusão para remover a nota
